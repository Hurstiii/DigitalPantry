...
async function instantiate_tables() {
  ...
  await Promise.all([db.run(create_products), db.run(create_tags)])
    .then(() => {
...
app.get("/products", (req, res, next) => {
  ...
  let tags = req.headers.tags; // get the tags boolean parameter as string ?!
  if (tags === "true") {
...
app.get("/products/:barcode", (req, res, next) => {
   ...
   */
  let tags = req.headers.tags; // get the tags boolean parameter
  if (tags === "true") {
    service.getProductsItemWithTags(db, barcode, (err, count, data) => {
      ...
      if (data.length === 0) {
        Log.log(
          `${log_base} Item not found in the database : searching external API`
        );
        request.get(
          ...
          function (err, extRes, body) {
            ...
            if (extRes.statusCode === 404) {
              Log.log(
                `${log_base} External API returned 404 : Product not in external database`
              );
              ...
              Log.log(
                `${log_base} External API returned 403 : Forbidden; meaning the api key has expired`
              );
              return res.status(404).json({ data: data }); // response indicating that the user needs to add the barcode as a new product
            } else {
              Log.log(body);
...
app.get("/pantry", (req, res, next) => {
  ...
  if (tags === "true") {
    service.getPantryWithTags(db, (err, count, data) => {
      if (err) {
        Log.log(`${log_base} ${err.errno} : ${err.message}`);
        return next(err);
      }
...
app.get("/pantry/:barcode", (req, res, next) => {
  ...
  let tags = req.headers.tags;
  if (tags === "true") {
    service.getPantryItemWithTags(db, barcode, (err, count, data) => {
      if (err) {
        Log.log(`${log_base} ${err.errno} : ${err.message}`);
        return next(err);
      }
  ...
  try {
    ...
    service.insertProducts(db, value, (err, count) => {
      ...
      res
        ...
        .send(
          `Rows inserted ${count}/${
            value.length === undefined ? 1 : value.length
          }`
        );
    });
  } catch (err) {
